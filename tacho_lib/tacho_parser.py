...
def iterate_test(dataclass, depth_symbol: str = " ", depth: int = 1):
    ...
    if type(dataclass) == tacho.cMF:
        print(type(dataclass).__name__)

    for field in fields(dataclass):
        if is_dataclass(field.type):
            if(field.type == tacho.cHeader):
...
def print_raw_data(data: bytearray | bytes):
...
def save_raw_data(data: bytearray | bytes):
...
def get_tacho_gen(data: bytearray | bytes):
    headers_list = do_analysis(data)
    for header in headers_list:
        if header.appendix > 1:
            return tacho.eTachoGen.Gen2.value
    return tacho.eTachoGen.Gen1.value
...
def print_analysis(data: bytearray | bytes):
    ...
    while idx < len(headers_list):
        ...
        if headers_list[idx].fid != headers_list[idx-1].fid:
            print("| {:<36} | {:<37} |".format(tacho.eFID.get_name(headers_list[idx].fid), ""))
        else:
        ...
        print("|   {:<34} | {:<37} |".format("Length:", int.from_bytes(headers_list[idx].data_length, "big")))
    ...
    print(" - {:<}: {:<}".format("Tachograph generation", get_tacho_gen(data)))
    print(" - {:<}: {:<}".format("Total headers found", len(headers_list)))
    print(" - {:<}:".format("EFs not present"))

    for fid in tacho.eFID:
        if fid.value["fid"] not in fid_list and not tacho.eFID.get_name(fid.value["fid"]).find("EF"):
            print(" -- {:<}".format(tacho.eFID.get_name(fid.value["fid"])))
...
def save_analysis(data: bytearray | bytes):
    ...
    with open(timestamp + " " + "Analysis Output.txt", 'w') as file:
        ...
        while idx < len(headers_list):
            ...
            if headers_list[idx].fid != headers_list[idx-1].fid:
                file.write("| {:<36} | {:<37} |\r".format(tacho.eFID.get_name(headers_list[idx].fid), ""))
            else:
            ...
            file.write("|   {:<34} | {:<37} |\r".format("Length:", int.from_bytes(headers_list[idx].data_length, "big")))
        ...
        file.write(" - {:<}: {:<}\r".format("Tachograph generation", get_tacho_gen(data)))
        file.write(" - {:<}: {:<}\r".format("Total headers found", len(headers_list)))
        file.write(" - {:<}:\r".format("EFs not present"))

        for fid in tacho.eFID:
            if fid.value["fid"] not in fid_list and not tacho.eFID.get_name(fid.value["fid"]).find("EF"):
                file.write(" -- {:<}\r".format(tacho.eFID.get_name(fid.value["fid"])))
